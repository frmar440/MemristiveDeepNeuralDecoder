...
def training_noise_run():
    STD = np.linspace(0.0, 0.05, 11)
    ...
    # resistive processing unit (for inference)
    rpu_config = InferenceRPUConfig()
    rpu_config.mapping = MappingParameter(digital_bias=False) # bias term is handled by the analog tile (crossbar)
    rpu_config.drift_compensation = None
    # non-idealities
    rpu_config.forward.inp_res = 1/256.  # infinite steps.
    rpu_config.forward.out_res = 1/256.  # infinite steps.
    ...
    columns = pd.MultiIndex.from_product([["p0015", "p0035", "p005", "p006", "p0065", "p007", "p0085", "p01"],
                                          ["mean", "std"]])
    df = pd.DataFrame(index=STD, columns=columns, dtype='float64')
    ...
    for DATA_PATH in DATA_PATHS:
        ...
        if pfr != "p01": # to REMOVE once all pfr are hwa-trained
            continue
        ...
        for std in STD:
            ...
            re_pfr_std = re.compile(f'{pfr}.*{std:.3f}')
    ...
    df.to_pickle('research/experiments/results/training_noise.pkl')
