"""inference optimal pdrop experiment
"""
import torch
import numpy as np
import pandas as pd
...
def inference_optimal_pdrop_run():
    ...
    # dataframe init
    columns = pd.MultiIndex.from_product([["fp-mdnd", "hwa-mdnd", "hhwa-mdnd"],
                                          ["mean", "std"]])
    df = pd.DataFrame(index=TEST_PDROPS, columns=columns, dtype='float64')

    # training scheme iteration (curves)
    for training_scheme in ["fp-mdnd", "hwa-mdnd", "hhwa-mdnd"]:
    ...
    df.to_pickle('research/experiments/results/hwa_inference_optimal_pdrop.pkl')
