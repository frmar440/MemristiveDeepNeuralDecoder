...
def dac_adc_resolution_run():
    ...
    df = pd.DataFrame(index=N_BITS, columns=columns, dtype='float64')
    ...
    for DATA_PATH, MDND_LOAD_PATH in zip(DATA_PATHS, MDND_LOAD_PATHS):
        ...
        data = []
        # ADC/DAC resolution iteration
        for n_bit in N_BITS:

            rpu_config.forward.inp_res = 1/(2**n_bit)  # n-bit DAC discretization.
            rpu_config.forward.out_res = 1/(2**n_bit)  # n-bit ADC discretization.
    ...
    df.to_pickle('research/experiments/results/dac_adc_resolution.pkl')
