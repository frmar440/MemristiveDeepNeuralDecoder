...
"""Phenomenological noise models for RRAM devices for inference."""
...
class RRAMLikeNoiseModel(BaseNoiseModel):
    r"""Noise model that was fitted and characterized on real RRAM devices.

    Expected weight noise at assumed time of inference with expected
    programming noise at 0.

    The statistical noise model is based on measured RRAM devices.
    ...
    Args:
        ...
        g_min: In :math:`\mu S`, the minimal conductance, ie the value
            the absolute min of the weights will be mapped to.
    ...
    def __init__(
        ...
        if T == 300:
            self.read_coeff = [0.5577, 0.11922, 0]
        elif T == 4:
            self.read_coeff = [0.3127, 0.13698, 0]
        else:
            raise ValueError('T must be either 300 or 4')

        self.prog_coeff = [0.0771, 1.036, 0] if prog_coeff is None else prog_coeff
        self.t_0 = t_0
        self.t_read = t_read
    ...
    def __str__(self) -> str:
        return ('{}(prog_coeff={}, read_coeff={}, g_converter={}, g_max={:1.2f}, g_min={:1.2f}, t_read={}, '
                't_0={:1.2f}, prog_noise_scale={}, '
                'read_noise_scale={}, drift_scale={})').format(
                    self.__class__.__name__, self.prog_coeff, self.read_coeff, self.g_converter,
                    self.g_max, self.g_min, self.t_read, self.t_0, self.prog_noise_scale,
