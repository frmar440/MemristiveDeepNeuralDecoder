...
enum class OutputWeightNoiseType { None, AdditiveConstant, PCMRead, RRAMRead };
  ...
  void printToStream(std::stringstream &ss) const {
    ...
    if (!is_perfect) {
      ...
      if (w_noise > 0.0 && w_noise_type != OutputWeightNoiseType::None) {
        ...
        if (w_noise_type == OutputWeightNoiseType::RRAMRead) {
          ss << "\t w_noise_type:\t\t" << (int)OutputWeightNoiseType::RRAMRead << " (RRAMRead) "
             << std::endl;
        }
      }
      if (ir_drop > 0.0) {
